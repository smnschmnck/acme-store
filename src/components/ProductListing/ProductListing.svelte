...
<div class="flex h-40 w-full justify-between gap-2 bg-white p-4">
	<div class="flex w-full gap-4">
		...
		<div class="flex h-full w-full flex-col justify-between truncate lg:w-fit">
			<div class="flex flex-col gap-0.5 text-sm">
				<a href={`/item/${id}`} class="truncate font-bold hover:underline">{name}</a>
			...
			<a
				...
				class="flex max-w-fit gap-2 rounded-lg bg-zinc-100 p-2 text-sm font-medium transition hover:bg-zinc-200"
			>
				<h5 class="truncate">
					{seller.name}
				</h5>
